...
jest.mock('@nestjs/swagger', () => ({
  ...
  ApiProperty: jest.fn(),
  SwaggerModule: {
    // El return value se utiliza cuando se desea validar el valor de retorno de la función en las pruebas.
...
describe('Main.ts', () => {
  ...
  it('should create the application with AppModule', async () => {
    ...
    expect(mockLogger.log).toHaveBeenCalledWith('App runing on port: 3000');
  });
  ...
  it('should create the application running on env.PORT', async () => {
    ...
    expect(NestFactory.create).toHaveBeenCalledWith(AppModule);
    expect(mockLogger.log).toHaveBeenCalledWith('App runing on port: 8000');
  });

  it('should set global prefix and enable CORS', async () => {
    await bootstrap();
  ...
  it('should create swagger document and setup', async () => {
    await bootstrap();
